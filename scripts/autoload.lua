...
local function scan_dir(path, current_file, dir_mode, separator, dir_depth, total_files, extensions)
    ...
    -- â†“ REPLACE alphanumsort(files) WITH THIS:
    local mtimes = {}
    for _, f in ipairs(files) do
        local info = utils.file_info(prefix .. f)
        mtimes[f] = info and info.mtime or 0
    end
    table.sort(files, function(a, b)
        return mtimes[a] > mtimes[b]  -- newest first
    end)
    -- â†‘ END REPLACEMENT

    alphanumsort(dirs)  -- keep this line, dirs still sort alphabetically
